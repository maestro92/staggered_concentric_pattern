...
#include "render_command_util.h"

//#include "debug_interface.h"
#include "memory.h"
#include "world.h"
#include "../staggered_concentric_pattern/asset.h"
#include "debug.h"
...
//	bool renderFlag = depth == 1;
	...
	if (renderFlag)
	{
		if (!node->IsLeafNode())
		{
			...
			{
				RenderCmdUtil::PushPlane(gameRenderCommands, group, bitmap, color, node->debugSplitPolygon.vertices, true);
				RenderCmdUtil::PushPlaneOutline(gameRenderCommands, group, bitmap, COLOR_GREEN, node->debugSplitPolygon.vertices);
			}
		}
	}
	...
	Entity* entity)
{
	...
	RenderCmdUtil::PushCube(gameRenderCommands, renderGroup, bitmap, COLOR_RED, min, max, true);
}
...
void RenderEntityStaticModel(GameRenderCommands* gameRenderCommands,
	...
	Entity* entity)
{
	AssetFamilyType::Enum assetFamilyType = entity->IsPatternCircle ? AssetFamilyType::Default : AssetFamilyType::Wall;

	BitmapId bitmapID = GetFirstBitmapIdFrom(gameAssets, assetFamilyType);
	LoadedBitmap* bitmap = GetBitmap(gameAssets, bitmapID);

	for (int i = 0; i < entity->model.size(); i++)
	{
		assert(entity->model[i].vertices.size() == 4);

		RenderCmdUtil::PushQuad(gameRenderCommands, renderGroup, bitmap,
...
//	if (!debugModeState->cameraDebugMode)
	...
	*group.quads = {};
	...
	BitmapId bitmapID = GetFirstBitmapIdFrom(gameAssets, AssetFamilyType::Default);
	LoadedBitmap* bitmap = GetBitmap(gameAssets, bitmapID);
	RenderCmdUtil::PushCoordinateSystem(gameRenderCommands, &group, bitmap, glm::vec3(0, 0, 0), glm::vec3(scale, scale, scale));
	...
	glm::ivec2 windowDimensions, DebugModeState* debugModeState)
{
	...
	DebugTable* globalDebugTable = gameMemory->debugTable;
	...
	if (!gameState->isInitalized)
	{
		...
		gameState->debugCameraEntity.pos = glm::vec3(0, 520, 520);
...
//	for (int i = 0; i < size; i++)
	...
	while (s[i] != '\0')
	{
		...
		else
		{
			...
			RenderCmdUtil::PushBitmap(gameRenderCommands, group, &glyphBitmap->bitmap, 
				COLOR_WHITE, leftTopPos, glm::vec3(width / 2.0, height / 2.0, 0), 
				RenderCmdUtil::AlignmentMode::Left, RenderCmdUtil::AlignmentMode::Top);
	...
	RenderGroup* renderGroup, GameAssets* gameAssets, glm::ivec2 mousePos)
{
	...
	// background
	RenderCmdUtil::PushBitmap(gameRenderCommands, renderGroup, defaultBitmap, glm::vec4(0, 0, 0.25, 0.25), profileRectMin,
		glm::vec3(profileRectDim.x / 2.0, profileRectDim.y / 2.0, 0), RenderCmdUtil::AlignmentMode::Left, RenderCmdUtil::AlignmentMode::Bottom);
	...
	if (debugState->mostRecentFrame != NULL)
	{
		...
		for (uint32 i = 0; i < root->children.size(); i++)
		{
			...
			RenderCmdUtil::PushBitmap(gameRenderCommands, renderGroup, defaultBitmap, color, glm::vec3(rectMin.x, rectMin.y, zOffset),
				glm::vec3(dim.x / 2.0, dim.y / 2.0, 0), RenderCmdUtil::AlignmentMode::Left, RenderCmdUtil::AlignmentMode::Bottom);
